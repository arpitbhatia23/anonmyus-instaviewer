      ...
      {post.data.items. map((items, index) => (
        <div key={index} className=' flex justify-center'>
          {items.carousel_media ? (
            <Carousel  showThumbs={false}   showStatus={false} className=' h-[400px] w-[350px] flex justify-center '>                      
           { items.carousel_media.map((carouselItem, carouselIndex) => (
              <div key={carouselIndex} className='flex   justify-center  '>
                <img src={carouselItem.thumbnail_url} alt='image' className='mt-4  text-center'/>
              </div>
            ))}</Carousel> 
